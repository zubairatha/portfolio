...
## 2) The zELO idea, step by step

* **Get candidates.** Retrieve top K docs for the query with BM25, embeddings, or a hybrid.

* **Plan a few smart matches.** Build a random k-regular graph over those K docs so each doc is compared to only a small, fixed number of neighbors.

* **Ask the judges.** For each selected pair ($d_i$, $d_j$), have a small LLM ensemble decide which doc better answers the query (and with what probability).

* **Record the outcomes.** Store these pairwise preferences in a sparse matrix (W) where observed pairs satisfy $w_{ij}$ + $w_{ji}$ = 1.

* **Create one ruler.** Fit Elo-style scores for all docs using a Bradley–Terry or Thurstone model so every candidate sits on a single, calibrated scale for this query.

* **Teach the student.** Train a pointwise cross-encoder on these absolute scores so it can predict calibrated relevance directly from $(q, d)$.

* **Use it in practice.** At inference: retrieve top K, score each $(q, d)$ once with the trained pointwise model, sort by score. No ensemble, no graph, no fitting.

That raises two immediate questions. 
1. How do we avoid paying for a quadratic number of LLM comparisons 
2. How do we turn separate, possibly miscalibrated LLM outputs into one consistent scoring scale The next two sections answer those.
...
## 4) Why these graph rules matter, not just how
...
## 5) From pairwise to absolute, with the right amount of math
...
## 6) The calibration problem that pairwise fixes
...
## 7) Training the pointwise cross-encoder, and the loss
